...
@router.post("/signup")
def signup(name:str,email:str,password:str, conn=Depends(get_conn)):
    hashed=hash_password(password)
    try:
        conn.execute(
            """
             INSERT INTO users (name,email,hashedPassword) VALUES(%s, %s,%s)
            """,
            (name,email,hashed)
        )
    except Exception:
...
def login(email:str,password:str, conn=Depends(get_conn)):
    user = conn.execute(
    "SELECT id, hashedPassword FROM users WHERE email=%s",
    (email,)
     ).fetchone()
